      ...
      if (highlighter) {
        ...
        if (meta) {
          ...
          try {
            ...
            console.error("Failed to parse meta data", (e as Error).message);
          }
      ...
      if (value.includes(`open_vscode`)) {
        ...
        if (anchorElm) {
          if (CONFIG.environment !== "production") {
            const stableCommand = anchorElm.getAttribute("href");
            const betaCommand = stableCommand?.replace(
              `eliostruyf.vscode-front-matter`,
              `eliostruyf.vscode-front-matter-beta`
            );
            anchorElm.setAttribute("href", betaCommand || "#");
          }
